...
const BlogView = () => {
    ...
    const selectedBlog = blog.find(blog => blog.id === Number(blogId))
    // 2) Safe defaults for local like state
    const [blogLike, setBlogLike] = useState(0)
    const [liked, setLiked] = useState(false)
    //const [blogLike, setBlogLike] = useState(selectedBlog?.likes.length)
    const { comment } = useSelector(store => store.comment)
    //const [liked, setLiked] = useState(selectedBlog?.likes.includes(user?.id) || false);
    const dispatch = useDispatch()
    console.log(selectedBlog);

    // 3) Once selectedBlog && user exist, initialize them
    useEffect(() => {
        if (selectedBlog && user) {
            // likes might be undefined or an array; default to empty array
            const likesArr = Array.isArray(selectedBlog.likes) ? selectedBlog.likes : []
            setBlogLike(likesArr.length)
            setLiked(likesArr.includes(user.id))
        }
    }, [selectedBlog, user])
    ...
    const likeOrDislikeHandler = async () => {
        try {
            ...
            const res = await axios.get(`http://localhost:8000/api/v1/blog/${selectedBlog?.id}/${action}`, { withCredentials: true })
            if (res.data.success) {
                ...
                <div className='mt-10'>
                    ...
                    <div className="flex items-center justify-between border-y dark:border-gray-800 border-gray-300 py-4 mb-8">
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <Button onClick={()=>handleShare(selectedBlog?.id)} variant="ghost" size="sm">
