...
const CreateBlog = () => {
    ...
    const getSelectedCategory = (value) => {
     ...
     // Track the newly created blog ID so we can wait for Redux to update before navigating
    ...
    const createBlogHandler = async () => {
        ...
        try {
            ...
            if (res.data.success) {
                /* const safeArray = Array.isArray(blog) ? blog : [];
                const created = res.data.blog
                // 1) Dispatch new array immediately
                dispatch(setBlog([...safeArray, created]))
                //navigate(`/dashboard/write-blog/${res.data.blog._id}`)
                setNewBlogId(created._id)

                const newBlog = res.data.blog
                const safeArray = Array.isArray(blog) ? blog : []
                dispatch(setBlog([...safeArray, newBlog])) */

                //setNewBlog(res.data.blog);

                const newBlog = res.data.blog
                const safeArray = Array.isArray(blog) ? blog : []
                const updatedArray = [...safeArray, newBlog];
                dispatch(setBlog(updatedArray));

                //dispatch(setBlog([...blog, res.data.blog]))
                // Immediately navigate—Redux has already updated synchronously.
                navigate(`/dashboard/write-blog/${res.data.blog.id}`)
    ...
    /* useEffect(() => {
        if (newBlogId) {
          const exists = blog.find((b) => b._id === newBlogId)
          if (exists) {
            navigate(`/dashboard/write-blog/${newBlogId}`)
          }
        }
      }, [newBlogId, blog, navigate]) */
