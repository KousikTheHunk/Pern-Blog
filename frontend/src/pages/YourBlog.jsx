...
const YourBlog = () => {
    ...
    const deleteBlog = async (id) => {
        try {
            ...
            if (res.data.success) {
                const updatedBlogData = blog.filter((blogItem) => blogItem?.id !== id);
                            ...
                            {blog?.map((item, index) => (
                                <TableRow key={index}>
                                    ...
                                    <TableCell className="flex gap-4 items-center">
                                        ...
                                        <h1 className='hover:underline cursor-pointer' onClick={() => navigate(`/blogs/${item.id}`)}>{item.title}</h1>
                                    </TableCell>
                                    ...
                                    <TableCell className="text-center">
                                        ...
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent className="w-[180px]">
                                                <DropdownMenuItem onClick={() => navigate(`/dashboard/write-blog/${item.id}`)}><Edit />Edit</DropdownMenuItem>
                                                <DropdownMenuItem className="text-red-500" onClick={() => deleteBlog(item.id)}><Trash2 />Delete</DropdownMenuItem>
                                            </DropdownMenuContent>
                                        </DropdownMenu>
                                    </TableCell>
                                </TableRow>
